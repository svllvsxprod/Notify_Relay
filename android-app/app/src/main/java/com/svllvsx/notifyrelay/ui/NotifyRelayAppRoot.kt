...
private fun Settings(container: AppContainer, padding: PaddingValues, requestSmsPermission: () -> Unit, s: UiStrings) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 20.dp), contentPadding = PaddingValues(vertical = 16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        item { SettingsCard(s.forwarding) { ToggleRow(s.notifications, settings.notificationForwardingEnabled) { scope.launch { container.settingsRepository.setNotificationForwarding(it) } }; ToggleRow(s.sms, settings.smsForwardingEnabled) { scope.launch { container.settingsRepository.setSmsForwarding(it); if (it) requestSmsPermission() } } } }
        item {
...
@Composable
private fun Diagnostics(container: AppContainer, padding: PaddingValues, s: UiStrings) {
    val settings by container.settingsRepository.settings.collectAsState(initial = com.svllvsx.notifyrelay.domain.model.AppSettings())
    val scope = rememberCoroutineScope()
    ...
    LazyColumn(Modifier.fillMaxSize().padding(padding).padding(horizontal = 20.dp), contentPadding = PaddingValues(vertical = 16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        item { StatusCard(Icons.Rounded.Notifications, s.notificationAccess, if (container.permissionsRepository.hasNotificationAccess()) s.enabled else s.noAccess, if (container.permissionsRepository.hasNotificationAccess()) RelayTone.Success else RelayTone.Warning) }
        item { StatusCard(Icons.Rounded.Sms, s.smsAccess, if (container.permissionsRepository.hasSmsPermission()) s.allowed else s.noAccess, if (container.permissionsRepository.hasSmsPermission()) RelayTone.Success else RelayTone.Warning) }
